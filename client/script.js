...
repeatBtn = document.querySelector('#repeat-button')
 ...
 repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/"randomParam"').then(response => {console.log(response.data)
...
document.querySelector("#query-button").addEventListener('click', queryFunction)
...
// CODE HERE 
createFood = () => {
    let foodInput = document.getElementById('inputArea').value
    body = {
        newFood: foodInput
    }
...
document.querySelector('#submitBtn').addEventListener('click', createFood)
